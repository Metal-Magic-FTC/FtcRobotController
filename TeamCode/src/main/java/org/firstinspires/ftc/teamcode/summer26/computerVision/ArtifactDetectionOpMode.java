...
public class ArtifactDetectionOpMode extends LinearOpMode {
    ...
    public static class ArtifactProcessor implements VisionProcessor {
        ...
        public Object processFrame(Mat frame, long captureTimeNanos) {
            ...
            // The FTC VisionPortal delivers frames as 4-channel RGBA Mats
            // (see org.firstinspires.ftc.vision.apriltag.AprilTagProcessorImpl,
            // which does Imgproc.cvtColor(input, grey, Imgproc.COLOR_RGBA2GRAY)).
            // Convert to 3-channel RGB before resizing so downstream buffer packing
            // assumptions (3 bytes/pixel) hold.
            Mat rgb = new Mat();
            ...
            // Convert RGB Mat → float32 RGB ByteBuffer (normalized 0-1)
            inputBuffer.rewind();
            ...
            for (int r = 0; r < rows; r++) {
                ...
                for (int c = 0; c < cols; c++) {
                    int base = c * 3;
                    // Mat is already RGB at this point — no channel swap needed
                    inputBuffer.putFloat(((rowBytes[base]     & 0xFF)) / 255.0f); // R
                    inputBuffer.putFloat(((rowBytes[base + 1] & 0xFF)) / 255.0f); // G
                    inputBuffer.putFloat(((rowBytes[base + 2] & 0xFF)) / 255.0f); // B
                }
            }
            resized.release();
        ...
        private List<Detection> parseAndNms(float[][] raw, int imgW, int imgH) {
            ...
            for (int a = 0; a < numAnchors; a++) {
                ...
                // Model outputs box coords already normalized 0-1 (Ultralytics
                // normalizes xywh by image size specifically for tflite export),
                // so just scale by the frame size — do not divide by inputSize.
                float cx = raw[0][a] * imgW;
                float cy = raw[1][a] * imgH;
                float bw = raw[2][a] * imgW;
                float bh = raw[3][a] * imgH;
